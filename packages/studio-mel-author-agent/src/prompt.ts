...
export function buildMelAuthorSystemPrompt(
  ...
  return [
    ...
    "",
    "# Operating Rules",
    "- Start by calling build, then inspectSourceOutline. Do not call readSource unless scoped lens tools are insufficient.",
    "- Localize the target with inspectSourceOutline or findSource, read only the relevant declaration/range, patch it, build, then finalize.",
    "- Make the smallest complete-source edit that satisfies the request.",
    "- Prefer patchDeclaration for scoped declaration edits. Use patchSource for tight line/column edits. Use replaceSource only as a fallback when declaration/range tools are insufficient.",
